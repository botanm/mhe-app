...
class NewPrivilegeScreen extends StatefulWidget {
  static const routeName = '/newprivilege';
  const NewPrivilegeScreen({this.args, super.key});
  final Map<String, dynamic>? args;

  @override
  _NewPrivilegeScreenState createState() => _NewPrivilegeScreenState();
}

class _NewPrivilegeScreenState extends State<NewPrivilegeScreen> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      if (args != null) {
        _editedElement =
            bpr.findBasicById('privilege', args['editedId'] as int);
        isViewOnly = args['isViewOnly'];
        title = isViewOnly
            ? i.tr('m53') + i.tr('privilege')
            : args['editedId'] != null
                ? i.tr('Edit')
                : title;
  ...
  Future<void> _onSubmit() async {
    ...
    try {
      await bpr.newBasic('privilege', _formData);
      Navigator.pop(
