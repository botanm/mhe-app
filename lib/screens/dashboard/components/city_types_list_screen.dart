...
import 'new_city_type_screen.dart';

class CityTypesListScreen extends StatefulWidget {
  static const routeName = '/citytypelist';
  const CityTypesListScreen({
    super.key,
  });

  @override
  State<CityTypesListScreen> createState() => _CityTypesListScreenState();
}

class _CityTypesListScreenState extends State<CityTypesListScreen> {
  late final i18n i;
  ...
  Widget build(BuildContext context) {
    print('*************** city_Types_list_screen build ***************');
    final List<dynamic> allElement = getPrioritizedElements(bpr.cityTypes);
      ...
      body: ListView.separated(
        ...
        itemBuilder: (context, index) => BasicsRUDListTile(
          ...
          onViewHandler: () {
            ...
            Utils.showNewScreen(context, args, NewCityTypeScreen.routeName,
                NewCityTypeScreen(args: args));
          },
          onEditHandler: () {
            final Map<String, dynamic> args = {
              'editedId': elements[index]['id'],
    ...
    return BasicsListAppBar(
      onAddHandler: () {
        if (isSmallSizeScreen) {
          Navigator.pushNamed(context, NewCityTypeScreen.routeName);
        } else {
          showDialog(
