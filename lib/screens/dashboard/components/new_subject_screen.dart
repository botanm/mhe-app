...
class NewSubjectScreen extends StatefulWidget {
  static const routeName = '/newsubject';
  const NewSubjectScreen({this.args, super.key});
...
class _NewSubjectScreenState extends State<NewSubjectScreen> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      title = i.tr('m52') + i.tr('subject');
      Map<String, dynamic>? args = widget.args;
      args ??=
          ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>?;

      if (args != null) {
        _editedElement = bpr.findBasicById('subject', args['editedId'] as int);
        ...
        _formData = {
          ...
          "category": _editedElement['category'],
  ...
  Widget build(BuildContext context) {
    print('+++++++++++    new_subject_screen build    +++++++++');
  ...
  MenuPicker get _buildCategory {
    return MenuPicker(
      allElements: _categories,
      maSecName: _maSecBasicName,
      initialSelected:
          _formData['category'] == "" ? [] : [_formData['category']],
  ...
  Future<void> _onSubmit() async {
    ...
    try {
      await bpr.newBasic('subject', _formData);
      Navigator.pop(
