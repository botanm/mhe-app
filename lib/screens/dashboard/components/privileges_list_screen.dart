...
import 'new_privilege_screen.dart';

class PrivilegesListScreen extends StatefulWidget {
  static const routeName = '/privilegeslist';
  const PrivilegesListScreen({
    super.key,
  });

  @override
  State<PrivilegesListScreen> createState() => _PrivilegesListScreenState();
}

class _PrivilegesListScreenState extends State<PrivilegesListScreen> {
  late final i18n i;
  ...
  Widget build(BuildContext context) {
    print('*************** privileges_list_screen build ***************');
    final List<dynamic> allElement = getPrioritizedElements(bpr.privileges);
      ...
      body: ListView.separated(
        ...
        itemBuilder: (context, index) => BasicsRUDListTile(
          ...
          onViewHandler: () {
            ...
            Utils.showNewScreen(context, args, NewPrivilegeScreen.routeName,
                NewPrivilegeScreen(args: args));
          },
          onEditHandler: () {
            final Map<String, dynamic> args = {
              'editedId': elements[index]['id'],
    ...
    return BasicsListAppBar(
      onAddHandler: () {
        if (isSmallSizeScreen) {
          Navigator.pushNamed(context, NewPrivilegeScreen.routeName);
        } else {
          showDialog(
              ...
              builder: (BuildContext context) {
                return Dialog(
                  ...
                  child: FractionallySizedBox(
                      ...
                      child: const NewPrivilegeScreen()),
                );
              });
        }
      },
