...
class NewTopicScreen extends StatefulWidget {
  static const routeName = '/newtopic';
  const NewTopicScreen({this.args, super.key});
...
class _NewTopicScreenState extends State<NewTopicScreen> {
  ...
  late final List<dynamic> _subjects;
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      _subjects = bpr.subjects;

      _maSecBasicName = i.maSecBasicName;

      title = i.tr('m52') + i.tr('topic');
      Map<String, dynamic>? args = widget.args;
      args ??=
          ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>?;

      if (args != null) {
        _editedElement = bpr.findBasicById('topic', args['editedId'] as int);
        isViewOnly = args['isViewOnly'];
        title = isViewOnly
            ? i.tr('m53') + i.tr('subject')
            : args['editedId'] != null
                ? i.tr('Edit')
                : title;
        ...
        _formData = {
          ...
          "subject": _editedElement['subject'],
  ...
  MenuPicker get _buildSubject {
    return MenuPicker(
      allElements: _subjects,
      maSecName: _maSecBasicName,
      initialSelected: _formData['subject'] == "" ? [] : [_formData['subject']],
  ...
  Future<void> _onSubmit() async {
    ...
    try {
      await bpr.newBasic('topic', _formData);
      Navigator.pop(
