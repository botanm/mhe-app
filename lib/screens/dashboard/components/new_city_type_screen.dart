...
class NewCityTypeScreen extends StatefulWidget {
  static const routeName = '/newcitytype';
  const NewCityTypeScreen({this.args, super.key});
  final Map<String, dynamic>? args;

  @override
  _NewCityTypeScreenState createState() => _NewCityTypeScreenState();
}

class _NewCityTypeScreenState extends State<NewCityTypeScreen> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      title = i.tr('m52') + i.tr('city type');
      Map<String, dynamic>? args = widget.args;
      args ??=
          ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>?;

      if (args != null) {
        _editedElement = bpr.findBasicById('cityType', args['editedId'] as int);
  ...
  Widget build(BuildContext context) {
    print('+++++++++++    new_city_type_screen build    +++++++++');
  ...
  Future<void> _onSubmit() async {
    ...
    try {
      await bpr.newBasic('cityType', _formData);
      Navigator.pop(
